...
class StringTest {
    ...
    private static final String CONTAINS_BLANK = "This is Java";
    private static final String CONTAINS_NON_BLANK = "ABCDE";

    @Test
    @DisplayName("substring(int beginIdx) : beginIdx 부터 나머지 문자열 출력")
    ...
    @Test
    @DisplayName("substring(int beginIdx) : beginIdx 가 문자열 길이와 같거나(공란 반환) 큰 경우(StringIndexOutOfBoundsException)")
    void substringWithBeginOnlyOverLen() {
        ...
        assertThrows(StringIndexOutOfBoundsException.class, () -> { // 큰 경우
            CONTAINS_NON_BLANK.substring(6);
        });
    }

...
public enum HttpMethodMapper {
    ...
    GET(GetMapping.class, HttpMethod.GET, method -> List.of(method.getAnnotation(GetMapping.class).value())),
    POST(PostMapping.class, HttpMethod.POST, method -> List.of(method.getAnnotation(PostMapping.class).value())),
    DELETE(DeleteMapping.class, HttpMethod.DELETE, method -> List.of(method.getAnnotation(DeleteMapping.class).value())),
    PUT(PutMapping.class, HttpMethod.PUT, method -> List.of(method.getAnnotation(PutMapping.class).value()));
    ...
    public static HttpMethod findHttpMethodByControllerMethod(Method method) {
        return Arrays.stream(values())
                .filter(httpMethod -> Objects.nonNull(method.getAnnotation(httpMethod.mappingAnnotation)))
                .map(httpMethod -> httpMethod.httpMethod)
                .findAny()
                .orElseThrow(() -> new IllegalArgumentException("매칭된 HTTP 메서드가 없습니다."));
    }
    ...
    public static List<String> extractMappingValues(Method method) {
        return Arrays.stream(values())
                ...
                .findAny()
                .orElseThrow(() -> new IllegalArgumentException("매칭되는 HTTP 메서드 어노테이션이 없습니다."));
    }
}